...
window.addEventListener('lowestPriceUpdate', (event) => {
  var { weapon, _previousLowestPrice, price, timestamp } = event.detail;
  // Parse the timestamp using moment
  weapon = weapon.replace("_", " ").split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')
...
window.addEventListener('removeWeaponFromGraph', function(event) {
    ...
    if (datasetIndex !== -1) {
        ...
        existingChartData._price_array = _price_array;
        existingChartData.colors = colors;
